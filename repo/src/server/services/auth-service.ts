...
function appendAudit(actorUserId: string | null, actionType: string, entityType: string, entityId: string, previousState: unknown, newState: unknown): void {
  const db = getDb();
  db.prepare(`
...
export function bootstrapAdmin(input: { username: string; password: string; confirmPassword: string }): void {
  if (!input.username || input.username.trim().length < 3) {
    throw Object.assign(new Error('Username must be at least 3 characters.'), { code: 'VALIDATION_ERROR' });
  }
  if (!input.password || input.password.length < 8) {
    throw Object.assign(new Error('Password must be at least 8 characters.'), { code: 'VALIDATION_ERROR' });
  }
  if (input.password !== input.confirmPassword) {
    throw Object.assign(new Error('Password confirmation does not match.'), { code: 'VALIDATION_ERROR' });
  }

  const db = getDb();
  const count = (db.prepare('SELECT COUNT(*) as cnt FROM users').get() as { cnt: number }).cnt;
  if (count > 0) {
    throw Object.assign(new Error('Administrator bootstrap has already been completed.'), { code: 'CONFLICT' });
  }
  ...
  db.prepare('INSERT INTO users (id, username, password_hash, roles, status, created_at) VALUES (?, ?, ?, ?, ?, ?)').run(
    userId, input.username.trim(), hash, roles, 'active', now
  );

  appendAudit(null, 'AUTH_BOOTSTRAP_ADMIN_CREATED', 'user', userId, null, { username: input.username, roles: ['Administrator'] });
}

export function login(input: { username: string; password: string }): { token: string; user: { id: string; username: string; roles: string[] } } {
  if (!input.username || !input.password) {
    throw Object.assign(new Error('Username and password are required.'), { code: 'VALIDATION_ERROR' });
  }
...
export function logout(token: string): void {
  ...
  if (session) {
    appendAudit(session.user_id, 'AUTH_LOGOUT', 'session', session.user_id, { status: 'authenticated' }, { status: 'logged_out' });
  }
}
