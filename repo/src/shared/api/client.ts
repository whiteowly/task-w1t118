...
async function request<T>(method: string, path: string, body?: unknown): Promise<T> {
  ...
  if (!res.ok) {
    const errorBody = await res
      .json()
      .catch(() => ({ error: { code: 'UNKNOWN_ERROR', message: res.statusText } }));
...
export const api = {
  ...
  rescheduleBooking: (
    bookingId: string,
    input: { resourceId: string; startsAt: string; durationMinutes: number; idempotencyKey: string }
