...
describe('auth service edge cases', () => {
  it('rejects bootstrap with username shorter than 3 chars', () => {
    expect(() =>
      bootstrapAdmin({ username: 'ab', password: 'password-123', confirmPassword: 'password-123' })
    ).toThrow();
  });

  it('rejects bootstrap with password shorter than 8 chars', () => {
    expect(() =>
      bootstrapAdmin({ username: 'admin', password: 'short', confirmPassword: 'short' })
    ).toThrow();
  });
  ...
  it('rejects login for non-existent user', () => {
    expect(() => login({ username: 'ghost', password: 'anything' })).toThrow(
  ...
  it('rejects login with wrong password', () => {
    ...
    expect(() => login({ username: 'admin', password: 'wrong-password' })).toThrow(
      'Invalid username or password.'
    );
  });
