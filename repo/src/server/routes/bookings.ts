...
router.get('/bookings/availability', requireAuth, requireCapability('workspace.booking.view'), (req: AuthenticatedRequest, res, next) => {
  try {
...
router.get('/bookings', requireAuth, requireCapability('workspace.booking.view'), (req: AuthenticatedRequest, res, next) => {
  try {
    const date = (req.query.date as string) || bookingService.todayDateKey();
...
router.post('/bookings', requireAuth, requireCapability('workspace.booking.manage'), (req: AuthenticatedRequest, res, next) => {
  try {
